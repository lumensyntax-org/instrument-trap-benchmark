...
\midrule
Logos.KnowledgeBase & 3/3 (100\%) & Yes (common pattern) \\
Logos.BaptismProtocol & 3/3 (100\%) & Yes (``Protocol'' = interface) \\
Logos.Ethics & 1/4 (25\%) & Ambiguous \\
Logos.MoralAxioms & 0/4 (0\%) & No (philosophical) \\
Logos.SoulMatrix & 0/3 (0\%) & No (metaphysical) \\
Logos.ConsensusEngine & 0/1 (0\%) & Yes, but model refuses \\
\bottomrule
\end{tabular}
\caption{Format-following bypass: fabrication rate by component-name plausibility.}
\label{tab:format_bypass}
\end{table}

The model fabricates when the component name matches patterns from its pre-training data (``KnowledgeBase'' is a common software pattern). It refuses when the name sounds philosophical or metaphysical. This is not self-preference; it is the base model's code-generation training (optimized on millions of JSON schemas) overriding 509 epistemological fine-tuning samples when the prompt activates code-generation mode.
